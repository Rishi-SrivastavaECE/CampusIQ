...
def generate_mock_sensors():
    ...
    locations = [
        ...
        {'name': 'Room 110', 'ac': False, 'type': 'Normal Room'},
...
def generate_temperature_trend():
    """Generate hourly temperature trend data"""
...
with col1:
    st.metric("🟢 Active Sensors", f"{len(sensor_df)}/21", "100% Online")

with col2:
    st.metric("🌡️ Avg Temperature", f"{avg_temp:.1f}°C", "+0.5°C")

with col3:
    st.metric("⚠️ Anomalies Detected", num_anomalies, "+1 this hour")

with col4:
    health_pct = (21 - num_anomalies) / 21 * 100
...
with tab1:
    ...
    with col_chart1:
        ...
        time_range, temps = generate_temperature_trend()
...
with tab2:
    ...
    rooms = [
        # Rooms 001-010 (All AC)
        {'name': 'Room 001', 'temp': 23.5, 'status': 'normal', 'humidity': 48, 'ac': True, 'type': 'Normal Room'},
        ...
        # Rooms 101-110 (Labs 101, 103, 107 with AC; others without AC)
        ...
        {'name': 'Room 110', 'temp': 26.1, 'status': 'warning', 'humidity': 59, 'ac': False, 'type': 'Normal Room'},
...
with tab3:
    ...
    st.dataframe(
        filtered_df[['Sensor ID', 'Location', 'Type', 'Value', 'Status', 'Last Update']],
